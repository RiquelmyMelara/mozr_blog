	...
	$.fn.accordionShortcodes = function(options) {
		...
		function clickHandler() {
			...
			if (!$(this).hasClass('open')) {
				...
				let myc = cl.replace('accordion-title','');
				myc = myc.replace('read','');

				$('.'+myc).next().clearQueue().stop().slideDown(duration, function() {
			...
			// If item is open, and click to close is set, close it
				...
				let cl = $(this).attr("class").replace(' ','');
				let myc = cl.replace('accordion-title','');
				myc = myc.replace('open','');
				myc = myc.replace('read','');
				console.log(myc);
				$('.'+myc).next().slideUp(duration);
				$('.'+myc).removeClass('open');
